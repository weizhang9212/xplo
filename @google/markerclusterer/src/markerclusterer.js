...
function MarkerClusterer(map, opt_markers, opt_options) {
  ...
  google.maps.event.addListener(this.map_, 'center_changed', function() {
    ...
    // time out for round show
    if (that.curMarkers_.length != that.clustersize_) {
      that.clustersize_ = that.curMarkers_.length;
      that.resetViewport();
    }
...
MarkerClusterer.prototype.roundInfoShow = function(){
  ...
  this.postShow = setInterval(function(){
    if(that.curMarkers_.length ===0) {
        return;
    }
    var index = that.roundIndex % (that.curMarkers_.length);
    that.curMarkers_[index].infowindow.open(that.map_, that.curMarkers_[index]);
    if(that.preShow !== null){
        that.preShow.infowindow.close();
    }
    that.preShow = that.curMarkers_[index];
    that.roundIndex += 1;
  },3000);
}
